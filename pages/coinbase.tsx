...
type Props = {
  initialUsers: CoinbaseVerifiedUser[]
  error?: string
}

export default function CoinbasePage({ initialUsers, error }: Props) {
  const router = useRouter()
  const { address, isConnected } = useAccount()
  const { signMessage } = useSignMessage()
  const [users, setUsers] = useState<CoinbaseVerifiedUser[]>(initialUsers)
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await fetch(`/api/coinbase/users?_=${timestamp}`, {
        cache: 'no-store',
        headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/coinbase/delete-airdrop', {
        method: 'POST',
        headers: {
  ...
  const handleVerify = async (isAutoVerifyFromSuccess = false) => {
    ...
    try {
      ...
      const response = await fetch('/api/coinbase/verify-token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div>
          ...
          <div style={{
            ...
            <p style={{
              ...
              Limited to Coinbase One members with active subscriptions. One claim per Coinbase account.
            </p>
